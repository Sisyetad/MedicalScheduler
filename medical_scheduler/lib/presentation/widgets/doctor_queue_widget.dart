...
import 'package:medical_scheduler/presentation/events/Doctor/doctor_queue_events.dart'; // import your DataQueue entity

class DoctorQueueWidget extends StatelessWidget {
  final List<DataQueue> queues;

  const DoctorQueueWidget({super.key, required this.queues});
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: () => context.go(
                          '/patient_history/${queue.patient.patientId}',
                        ),
                        style: ElevatedButton.styleFrom(
                          backgroundColor: const Color(0xFF2B5F91),
                          shape: RoundedRectangleBorder(
                      ...
                      child: Consumer(
                        builder: (context, ref, child) => ElevatedButton(
                          onPressed: () => ref
                              .read(doctorQueueNotifierProvider.notifier)
                              .mapEventToState(
                                UpdateQueueStatus(queue.queueId, 3),
                              ),
                          style: ElevatedButton.styleFrom(
                            backgroundColor: const Color(0xFF2B5F91),
                            shape: RoundedRectangleBorder(
